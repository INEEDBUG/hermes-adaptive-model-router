...
> 说明：本文所有表述均以项目当前实际完成度为准。系统当前处于 **Production Shadow 已验证 / validated in shadow mode** 阶段：路由决策被真实采集并记录，实际执行的模型仍由网关自身配置决定，自动切换尚未启用。v0.2.0 新增一条结构性的人类回合来源边界（human-turn provenance boundary）：只有 human-origin 回合才会进入路由观测。文中不含任何成本、性能或规模量化结论。
...
基于 Hermes Agent v0.21.5 的隐私保护自适应 LLM 路由层（Shadow Router），通过单一 `pre_api_request` 插件钩子接入 TypeSafe JEV 决策服务，在快速低成本模型（DeepSeek V4.1 Flash）与高能力模型（MiMo V2.6 Pro）之间给出逐轮路由决策。系统以 Shadow 模式在真实消息网关流量上验证：决策被完整采集与统计，实际执行模型保持不变。设计上以数据最小化、fail-open 与可逆的运行时 kill switch 为约束。集成本身是插件式的：Shadow 路由逻辑全部位于插件侧，而严格的人类回合来源判定需要一份最小的 Hermes v0.21.5 `turn_origin` 集成补丁（v0.2.0）。

## v0.2.0 增量：人类回合来源边界（human-turn provenance boundary）

v0.2.0 处理的是一个**从真实 telemetry 里诊断出来的隐私问题**，不是设计阶段的假想：观察钩子对所有 provider 请求都会触发，而网关自注入的通知、compaction 续跑、background-review fork 与 subagent 回合都可能**继承**它们所来源会话的 platform 标签 —— 一次飞书会话的 background review 一样上报 `platform=feishu`，所以"平台在 allowlist 里"根本说明不了这一轮来自人类。修复方式是**结构性的**：在**任何 Routing Dossier 构造之前**先过两道门 —— 门 1：`platform IN JEV_ALLOWED_PLATFORMS`；门 2（权威条件）：`turn_origin == "user"`。任一不满足即拒绝该回合，发生在 `redact → dossier.build → shadow.submit` 之前；来源判定**从不使用消息文本**启发式。

`turn_origin` 由一份**最小的 Hermes 集成补丁**提供（`patches/hermes-v0.21.5-turn-origin.patch`，针对 Hermes Agent v0.21.5 构建与测试），取值来自封闭词表：`user`、`internal_notification`、`compaction_continuation`、`background_review`、`subagent`、`oneshot`、`cron`、`curator`、`api_server`、`unknown`。缺失、空或 unknown 的来源一律**拒绝**，绝不默认成 `user`（fail-closed）。

验证（全部离线或脱敏，不碰冻结的生产系统）：

- 离线矩阵 `tests/test_turn_boundary.py`：37 项检查、分 A–G 组（双门矩阵、fail-closed 路径、按 turn 计数、并发隔离、content-free telemetry、异常检测器、记录中的来源字段）；
- 并发隔离测试：人类回合与 background 回合并行跑，互不污染（人类回合被观测，background 回合为 0）；
- content-free telemetry 审计：拒绝记录只写 date / platform / turn_origin / reason / count，写入路径**没有 message 参数**；
- 脱敏生产 smoke：人类回合 → 1 条路由决策；subagent、internal notification、background review、缺失/unknown 来源 → 均为 0；并发 user/background 隔离 PASS；content-free telemetry PASS；
- 上游漂移防护：`tools/check_turn_origin_patch.py` 校验集成（版本目标、补丁标记、插件的两道门、离线 fail-closed 证明），检测到漂移时 exit 3，路由器保持 fail-closed/关闭，直到补丁被重新应用。
...
> 不声明：路由准确率、成本节省、任何自动切换收益，以及 shadow 观测之外的任何生产影响。
...
- **多模型路由架构设计**：以单个 `pre_api_request` 观察者钩子接入 Agent 运行时，无 fork、无代理层；Shadow 路由逻辑完全位于插件侧，v0.2.0 的严格来源判定另需一份最小的 `turn_origin` 集成补丁（见上节）；每轮仅在首次 LLM 调用时触发一次路由，重试与后续 tool-loop 迭代显式跳过；决策服务严格为咨询性质，钩子始终返回 `None`，不注入上下文、不修改请求内容。
...
- **隐私脱敏与 Routing Dossier**：确定性本地正则脱敏（私钥块、带前缀的 API Key、Bearer Token、`key=value` 密钥、邮箱、IPv4/IPv6、`user@host`、长随机串），不依赖模型与网络；Dossier 只包含当前轮脱敏后截断的文本、布尔任务/风险标记与可用路由列表，不包含历史、长期记忆、工具输出、系统提示词与凭证；检出私钥类内容时整轮不发出，改记本地 Local Privacy Fallback（隐私 fail-closed，可用性 fail-open）；telemetry 为 content-free，字段白名单由测试强制；v0.2.0 再加一层结构性边界：只有 human-origin 回合会被观测，platform allowlist 单独并不构成隐私边界。
...
为 Hermes Agent 构建的隐私保护自适应多模型路由层，以单一插件钩子接入（Shadow 路由逻辑零核心改动；严格人类回合来源另需一份最小 `turn_origin` 集成补丁），通过 Shadow 模式在真实流量上完成决策采集与验证，并以数据最小化、fail-open 与可回退的运行时 kill switch 保证 Agent 回合安全。

## 精简版 bullet（3 条）

- 为 Hermes Agent v0.21.5 设计并落地隐私保护的多模型自适应路由层（Shadow Router），通过单个 `pre_api_request` 插件钩子接入 TypeSafe JEV 决策服务，在 DeepSeek V4.1 Flash 与 MiMo V2.6 Pro 之间输出逐轮路由决策，Shadow 路由逻辑零核心改动、无代理层（v0.2.0 的严格人类回合来源另需一份最小 `turn_origin` 集成补丁）。
...
- 建立完整可靠性体系：全路径 fail-open 错误分类（timeout / HTTP 401、429、5xx / 畸形响应）、状态文件 + `KILL` 哨兵的运行时 kill switch（按轮解析、fail-safe = off）、真实/测试样本隔离统计；39/39 离线检查通过（live 组需 RUN_LIVE_TESTS=1 显式开启，开启后 46/46）、26/26 kill switch 检查通过，另有 v0.2.0 人类回合来源边界 37/37 离线检查通过、secret-scanner 控制项 24/24 与 installer 36/36 两个套件通过。

## 完整版 bullet（5 条）

- 独立设计并落地面向 Hermes Agent v0.21.5 的隐私保护自适应 LLM 路由层（Shadow Router），通过唯一一个 `pre_api_request` 插件钩子接入 TypeSafe JEV 决策服务（`POST /v1/systemone`，choice 问题），在快速低成本模型 DeepSeek V4.1 Flash 与高能力模型 MiMo V2.6 Pro 之间给出逐轮路由决策，并记录决策服务返回的具体模型版本以保证可复现；Shadow 路由逻辑零核心改动、每轮仅首次调用触发一次；v0.2.0 的严格人类回合来源另需一份最小的 Hermes v0.21.5 `turn_origin` 集成补丁。
...
- 构建隐私优先的数据通路：确定性本地正则脱敏（私钥、带前缀 API Key、Bearer Token、`key=value` 密钥、邮箱、IPv4/IPv6、`user@host`、长随机串），Dossier 仅含当前轮脱敏截断文本与布尔任务/风险标记，排除历史、记忆、工具输出与凭证；检出不可安全脱敏内容时整轮不发出并记录 Local Privacy Fallback；telemetry 字段白名单由测试强制，凭据与请求头永不落日志；v0.2.0 起再加一层结构性边界：只有 human-origin 回合会被观测，且判定在任何 Dossier 构造之前完成。
...
- v0.2.0：从真实 telemetry 诊断出"platform 相同并不代表来源是人类"，改为结构性修复 —— 在 Dossier 构造前用两道门判定来源（platform allowlist + 核心提供的 `turn_origin == "user"`），来源标签由一份最小集成补丁提供，缺失/未知一律 fail-closed 拒绝，并用离线矩阵、并发隔离测试、content-free telemetry 审计与脱敏生产 smoke 验证。
...
- 路由检查 39/39 离线检查通过（live 分组默认跳过，需 RUN_LIVE_TESTS=1 与凭证显式开启，开启后 46/46）；kill switch 解析 26/26 通过；v0.2.0 人类回合来源边界 37/37 离线检查通过（A–G 组）；secret-scanner 控制项 24/24 与 installer 36/36 两个套件通过，CI 每次 push 都运行全部四个套件、状态初始化 dry run 与密钥/隐私扫描，无需配置任何 secret。
...
- v0.2.0：人类回合来源边界落地并通过验证 —— 37/37 离线矩阵（双门矩阵、fail-closed、按 turn 计数、并发隔离、content-free telemetry、异常检测器、记录中的来源字段）、脱敏生产 smoke（人类回合 1 条决策，subagent / internal notification / background review / 缺失或未知来源均为 0）、`tools/check_turn_origin_patch.py` 漂移检查（exit 3 即保持 fail-closed）。
- 明确边界：自动切换处于设计阶段、未启用；自动模式所需的负载下并发隔离实测仍未做（v0.2.0 完成的是来源层面的并发隔离测试）；不发布任何成本、性能或规模量化结论；v0.2.0 不声明路由准确率、成本节省或任何自动切换收益，也不声明 shadow 观测之外的任何生产影响。

## English Resume Version

### Project entry (2–3 lines)

Privacy-preserving adaptive LLM routing layer (Shadow Router) for Hermes Agent v0.21.5, integrated through a single `pre_api_request` plugin hook. It asks a TypeSafe JEV decision service for a per-turn route choice between a fast/cost-efficient model (DeepSeek V4.1 Flash) and a high-capability model (MiMo V2.6 Pro), then validates the decision quality in shadow mode on live messaging-gateway traffic while the executing model stays unchanged. Built around data minimisation, fail-open behaviour and a reversible runtime kill switch, with the shadow routing logic itself entirely plugin-based and no routing proxy; strict human-turn provenance additionally requires a minimal Hermes v0.21.5 `turn_origin` integration patch.
...
- Designed and built a privacy-preserving adaptive LLM routing layer for Hermes Agent v0.21.5 using a single `pre_api_request` plugin hook and a TypeSafe JEV decision service, producing per-turn route decisions between DeepSeek V4.1 Flash and MiMo V2.6 Pro with no routing proxy and the shadow routing logic entirely plugin-based; strict human-turn provenance additionally requires a minimal Hermes v0.21.5 `turn_origin` integration patch.
...
### v0.2.0 increment — human-turn provenance boundary

v0.2.0 fixes a privacy problem diagnosed from real telemetry rather than from a design review. The observer hook fires for every provider request the agent makes, and gateway self-injected notifications, compaction continuations, background-review forks and subagent turns can inherit the platform label of the session that spawned them (a background review of a Feishu conversation reports `platform=feishu`), so a platform allowlist alone cannot separate a human turn from an internal one.

The fix is structural, not heuristic. Two gates are evaluated before any Routing Dossier is constructed: gate 1, `platform IN JEV_ALLOWED_PLATFORMS`; gate 2 (authoritative), `turn_origin == "user"`. Failing either rejects the turn before `redact → dossier.build → shadow.submit`, and message text is never used to decide origin. The label comes from a minimal Hermes integration patch (`patches/hermes-v0.21.5-turn-origin.patch`, built and tested against Hermes Agent v0.21.5) with a closed vocabulary (`user`, `internal_notification`, `compaction_continuation`, `background_review`, `subagent`, `oneshot`, `cron`, `curator`, `api_server`, `unknown`); missing, empty or unknown origin is rejected and never defaulted to `user` (fail-closed).

Verification, all offline or sanitised: an offline matrix (`tests/test_turn_boundary.py`, 37 checks in groups A–G — dual-gate matrix, fail-closed paths, per-turn counting, concurrency isolation, content-free telemetry, anomaly detector, provenance in the record); a concurrency isolation test running human and background turns in parallel; a content-free telemetry audit (date, platform, turn_origin, reason, count only — the write path takes no message argument); and a sanitised production smoke test (human turn → 1 routing decision; subagent, internal notification, background review, missing/unknown origin → 0; concurrent user/background isolation PASS; content-free telemetry PASS). `tools/check_turn_origin_patch.py` guards against upstream drift: exit 3 means drift, and the router stays fail-closed/off until the patch is re-applied.

> The router only observes human-origin turns. Internal, subagent, background and continuation turns are rejected before Routing Dossier construction.

> Shadow routing logic remains plugin-based, while strict human-turn provenance requires a minimal Hermes v0.21.5 turn_origin integration patch.

> Not claimed: routing accuracy, cost savings, any automatic-switching benefit, or any production impact beyond shadow observation.

> Built and tested against Hermes Agent v0.21.5. Hermes Agent is a Nous Research project; this repository is an independent plugin project with no affiliation to it.

### 60-second spoken pitch

I built a multi-model adaptive routing and reliability layer for Hermes Agent. The problem: an agent that runs long tool-calling loops on one model always pays the wrong price, and the usual fix places a third-party router in the request path, which means full prompts and credentials leave the host and you gain a new single point of failure. So I integrated a routing decision service through one `pre_api_request` plugin hook: one decision per turn, built from a dossier that contains only the current turn's redacted, truncated text plus boolean task and risk flags, never history, memory, tool output or credentials. Three engineering decisions mattered most. First, shadow first: decisions are collected and recorded while the executing model stays exactly as configured, so decision quality is proven on real traffic before anything acts on it. Second, the whole routing path is fail-open: timeouts, 401s, 429s, 5xx and malformed responses are classified and dropped - verified through fault injection and mocked error classification rather than by causing real production failures - and when one genuine routing timeout occurred in production traffic the reply was still delivered normally. Third, the runtime kill switch is a state file plus a `KILL` sentinel, resolved every turn with fail-safe off, locked down by 26 checks. Today the status is validated in shadow mode: 39/39 offline checks; 46/46 with the live routing group enabled via RUN_LIVE_TESTS=1; 26/26 kill-switch checks, plus a secret-scanner control suite (24/24), an installer suite (36/36) and the v0.2.0 human-turn boundary suite (37/37); and the alternative provider passed a six-item validation matrix. Route availability is explicit configuration rather than a hard-coded constant, so with nothing configured the shadow record's would_execute is null, and while the KILL sentinel exists the state initialiser refuses and cannot write state at all. Automatic switching is designed but not enabled, and concurrency isolation is analysed but not yet empirically tested — I state that boundary plainly rather than implying it works.

## 使用注意

以上表述严格对应项目当前完成度：自动切换（Auto）尚未启用，Shadow 模式不改变实际执行模型，所有未实测项已显式标注；v0.2.0 的人类回合来源边界（只有 human-origin 回合会被观测，判定在任何 Dossier 构造之前、fail-closed）已通过离线矩阵、并发隔离测试、content-free telemetry 审计与脱敏生产 smoke 验证，v0.1.x 中"零核心改动"的说法已按事实修正：Shadow 路由逻辑仍是插件式的，严格人类回合来源需要一份最小的 `turn_origin` 集成补丁。仍未声明：路由准确率、成本节省、自动切换收益、shadow 观测之外的任何生产影响。后续若积累足够的真实流量数据，再补充量化指标。
